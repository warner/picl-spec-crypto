...
def create_salted_verification_key( username, password, hash_alg=SHA1, ng_type=NG_2048, n_hex=None, g_hex=None, forced_salt=None ):
    if ng_type == NG_CUSTOM and (n_hex is None or g_hex is None):
        raise ValueError("Both n_hex and g_hex are required when ng_type = NG_CUSTOM")
    hash_class = _hash_map[ hash_alg ]
    N,g = get_ng( ng_type, n_hex, g_hex )
    if forced_salt:
        assert type(forced_salt) == type(os.urandom(1))
        _s = forced_salt
    else:
        _s = os.urandom(4)
...
class Verifier (object):
    ...
    def __init__(self, username, bytes_s, bytes_v, bytes_A, hash_alg=SHA1, ng_type=NG_2048, n_hex=None, g_hex=None, force_b=None):
        if ng_type == NG_CUSTOM and (n_hex is None or g_hex is None):
            raise ValueError("Both n_hex and g_hex are required when ng_type = NG_CUSTOM")
        self.s = bytes_s
        ...
        if not self.safety_failed:
            if force_b is None:
                ...
                assert type(force_b) == type(os.urandom(1))
                self.b = bytes_to_long(force_b)
            ...
            #print "v-N", N
            #print "v-g", g
            #print "v-I", self.I.encode("hex")
            #print "v-k", k
            #print "v-v", self.v
            #print "v-u", self.u
            #print "v-s", self.s.encode("hex")
            #print "v-S", self.S
            #print "v-A", self.A
            #print "v-B", self.B
            #print "v-K", self.K.encode("hex")
    ...
    def verify_session(self, user_M):
        #print "v.VERIFY_SESSION", self.safety_failed, self.M.encode("hex"), user_M.encode("hex")
...
class User (object):
    def __init__(self, username, password, hash_alg=SHA1, ng_type=NG_2048, n_hex=None, g_hex=None, force_a=None):
        if ng_type == NG_CUSTOM and (n_hex is None or g_hex is None):
            raise ValueError("Both n_hex and g_hex are required when ng_type = NG_CUSTOM")
        N,g        = get_ng( ng_type, n_hex, g_hex )
        hash_class = _hash_map[ hash_alg ]
        ...
        if force_a is None:
            ...
            assert type(force_a) == type(os.urandom(1))
            self.a = bytes_to_long(force_a)
    ...
    def process_challenge(self, bytes_s, bytes_B):
        ...
        #print "c-N", N
        #print "c-g", g
        #print "c-I", self.I.encode("hex")
        #print "c-p", self.p.encode("hex")
        #print "c-x", self.x
        #print "c-k", k
        #print "c-v", self.v
        #print "c-u", self.u
